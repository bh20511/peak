...
//查詢該筆訂單的房間單價
$sql = "SELECT `room_price` FROM `room` join `booking_order` on `room`.`room_sid` = `booking_order`.`room_sid` WHERE `order_num`= $num";
$sql_stmt = $pdo->query($sql)->fetch();

//重新計算總額
$total = $sql_stmt['room_price'] * $_POST['qty'];

//更新
$p_sql = "UPDATE `booking_order` SET 
    `qty`= ?,
    `total`=?
    WHERE `order_num` = $num";
