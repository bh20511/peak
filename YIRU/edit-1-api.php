...
//查詢該筆訂單的商品單價
$sql = "SELECT `product_price` FROM `product` join `product_order` on `product`.`product_sid` = `product_order`.`products_sid` WHERE `order_num`= $num";
$sql_stmt = $pdo->query($sql)->fetch();

//重新計算總額
$total = $sql_stmt['product_price'] * $_POST['qty'];

//更新
$p_sql = "UPDATE `product_order` SET 
    `qty`= ?,
    `total`=?
    WHERE `order_num` = $num";
